...
def update_memory(store, namespace, messages):
    ...
    user_preferences = store.get(namespace, "user_preferences")
    # Update the memory
    llm = ChatGoogleGenerativeAI(model="gemini-2.0-flash-exp", model_provider="google-genai", max_tokens=32000, temperature=0.0).with_structured_output(UserPreferences)
    result = llm.invoke(
        [
